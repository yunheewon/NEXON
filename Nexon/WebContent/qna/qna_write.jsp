...
<head>
    ...
    <title>문의작성</title>
    <link rel="stylesheet" href="${contextPath}/css/qna_write.css">
    ...
    <script type="text/javascript" src="${contextPath}/js/qna_write.js"></script>
</head>
<body>
    <!--gnb_wrapper 영역-->
    ...
    <div id="write_wrapper">
        <h1>문의작성</h1>
        <!--write_form 영역-->
        <form class="write_form" action="${contextPath}\WebSendMail" method="post" enctype="multipart/form-data">
        <!-- multipart/form-data는 form태그 안에서 post와 같은 방식으로 여러가지 데이터를 함께 묶어 전송하는 경우 사용 -->
            <h3>아래 작성한 이메일로 답변이 전달됩니다.</h3>
            <!--return_email 영역-->
            <div class="return_email">
                <label for="email">답변 수신<br>메일주소</label>
                <input type="email" name="email" id="email">
            </div>
            <!--//return_email 영역-->
            <!--game_sele 영역-->
            <div class="game_sele">
                <label for="game">게임선택</label>
                <select name="game" id="game">
                    <option value="게임선택">게임선택</option>
                    <option value="던전앤파이터">던전앤파이터</option>
                    <option value="메이플스토리">메이플스토리</option>
                    <option value=엘소드>엘소드</option>
                    <option value="클로저스">클로저스</option>
                </select>
            </div>
            <!--//game_sele 영역-->
            <!--qna_kind 영역-->
            <div class="qna_kind">
                <label for="kind">문의분류</label>
                <select name="kind" id="kind">
                    <option value="문의분류">문의분류</option>
                    <option value="버그">버그</option>
                    <option value="거래">거래</option>
                    <option value="가입 및 탈퇴">가입 및 탈퇴</option>
                </select>
            </div>
            <!--//qna_kind 영역-->
            <!--qna_content 영역-->
            ...
            <!--add_file 영역-->
            <div class="add_file">
                <label for="file_tit" class="file_tit">첨부파일</label>
                <input class="upload-name" value="파일선택" disabled="disabled">
                <input type="file" id="ex_filename" class="upload-hidden" name="attachment">
                <label for="ex_filename" >파일선택</label>
                <label for="del_filename" id="del_filename">파일삭제</label>
            </div>
            <!--//add_file 영역-->
            <!--waring_msg 영역-->
            <ul class="warning_msg">
                <li>개인정보 서류는 접수하지 않습니다.</li>
                <li>문의[제목]과 [내용]란에는 절대 개인정보를 입력하지 마세요.</li>
                <li>문의[제목]과 [내용]에 개인정보가 포함되어있거니, 중복 문의인 경우에는 삭제될 수 있습니다.</li>
            </ul>
            <!--//waring_msg 영역-->
            <!--btn_wrap 영역-->
            <div class="btn_wrap">
